# Parse JSONL transcript and output formatted markdown conversation
...
$markdown += "# ประวัติบทสนทนาระบบตรวจสอบพัสดุ (AssetFlow)"
$markdown += ""
$markdown += "บทสนทนานี้เป็นบทสนทนาการพัฒนาระบบ AssetFlow (ระบบบริหารจัดการและติดตามครุภัณฑ์ประจำปีด้วย QR Code และ PDF)"
$markdown += ""
...
foreach ($line in $lines) {
    ...
    if ($obj.type -eq "USER_INPUT") {
        ...
        if ($content) {
            $markdown += "## 🤖 ผู้ช่วย AI (Antigravity) - รอบที่ $turnCount"
            $markdown += $content
            $markdown += ""
            $markdown += "---"
            $markdown += ""
...
Write-Output "Successfully parsed and saved to $outputPath"
